...
/**
 * Description of Personiluser
 */
class Personiluser extends CI_Controller {
    ...
    public function index() {
        if($this->session->userdata('loggeduser')){
            ...
            // mencari nama dan pangkat personil
            $jml = $this->Mglobals->getAllQR("SELECT count(*) as jml FROM personil where nrp = '".$ses['nrp']."';")->jml;
            if($jml > 0){
                $tersimpan = $this->Mglobals->getAllQR("SELECT * FROM personil a, pangkat b where a.idpangkat = b.idpangkat and a.nrp = '".$ses['nrp']."';");
                $data['id_personil'] = $tersimpan->idpersonil;
                $data['nm_personil'] = $tersimpan->nama;
                $data['pangkat_personil'] = $tersimpan->nama_pangkat;
            }else{
                $data['id_personil'] = "";
            ...
            $this->load->view('personil_user/keluarga');
            $this->load->view('foot');
        }else{
