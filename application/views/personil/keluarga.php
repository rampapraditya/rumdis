<script type="text/javascript">
    ...
    $(document).ready(function () {
        table = $('#tb').DataTable({
            ajax: "<?php echo base_url(); ?>personil/ajaxkeluarga/<?php echo $tersimpan->idpersonil; ?>",
            ordering:false
        });
    });
    ...
    function save() {
        ...
        if (nama === '') {
            ...
            var url = "";
            if (save_method === 'add') {
                url = "<?php echo base_url(); ?>personil/ajax_add_keluarga";
            } else {
                url = "<?php echo base_url(); ?>personil/ajax_edit_keluarga";
            }
            $.ajax({
                url: url,
                type: "POST",
    ...
    function hapus(id, no) {
        if (confirm("Apakah anda yakin menghapus keluarga nomor " + no + " ?")) {
            $.ajax({
                url: "<?php echo base_url(); ?>personil/hapuskeluarga/" + id,
                type: "POST",
                dataType: "JSON",
                success: function (data) {
                    iziToast.success({
    ...
    function ganti(id) {
        ...
        $.ajax({
            url: "<?php echo base_url(); ?>personil/gantikeluarga/" + id,
            type: "POST",
            dataType: "JSON",
            success: function (data) {
                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="form-group">
                                <label>NRP</label>
                                <input type="text" class="form-control" autocomplete="off" readonly value="<?php echo $tersimpan->nrp; ?>">
                            </div>
                            <div class="form-group">
                                <label>PERSONIL</label>
                                <input type="text" class="form-control" autocomplete="off" readonly value="<?php echo $tersimpan->nama_pangkat.' '.$tersimpan->nama; ?>">
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="form" class="form-horizontal">
                    ...
                    <input type="hidden" name="idperonil" id="idperonil" value="<?php echo $tersimpan->idpersonil; ?>">
                    <div class="form-body">
                        <div class="form-group">
